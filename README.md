...
- **طول پین دانش‌آموز**: سمت کلاینت (فرم‌های ورود/ثبت‌نام/تغییر پین در `student/`، و تولید پین در `admin-desktop`) به ۶ کاراکتر (حداقل یک حرف بزرگ و یک حرف کوچیک انگلیسی) به‌روزرسانی شد. اما تابع/RPC سمت دیتابیس (`student_login`, `student_login_or_register`, `student_change_own_pin`, `staff_reset_student_pin`) باید جدا در داشبورد Supabase بررسی بشه تا مطمئن بشیم محدودیت طول ثابتی (مثل چک عددی‌بودن) جلوی پین‌های جدید رو نمی‌گیره.
- **ذخیره‌ی محلی اطلاعات ورود برای «ورود سریع با بیومتریک»**: `shared/js/biometric.js` به‌صراحت اطلاعات ورود (شماره+پین برای دانش‌آموز، ایمیل+رمز برای مربی/ادمین) را بدون رمزنگاری در `localStorage` نگه می‌داره؛ WebAuthn فقط جلوی بازکردن UI رو می‌گیره، نه خواندن مستقیم localStorage. اگه امنیت این بخش برای پروژه مهمه، نیاز به رمزنگاری (مثلاً با WebAuthn PRF) یا حذف این قابلیت داره.
